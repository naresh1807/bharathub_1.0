...
// నేరుగా EmployerLoginView.post() కి సబ్మిట్ అవుతుంది, కాబట్టి
// ఇంతకుముందు ఉన్న fake "2FA OTP" మోడల్ ఫంక్షన్లు (doLogin/
// openEmpOtpModal/verifyLoginOTP వంటివి) తీసివేశాం -- అవి ఇక ఏ
...
function verifyIdentity() {
  ...
  if (!email || !mobile || !empId) {
    showAlert('verifyAlert', 'Please enter Corporate Email, Mobile, and Employer ID — all fields!');
    return;
  }
  ...
  fetch('/accounts/password/forgot/verify/', {
    ...
    body: JSON.stringify({ role: 'employer', email, mobile, employer_id: empId }),
  })
    .then(r => r.json())
    .then(data => {
...
function moveFOtp(input, idx) {
  ...
  const inputs = document.querySelectorAll('#forgotOtpRow .otp-digit');
